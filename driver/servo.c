...
void SERVO_SetAngle(uint8_t channel, float angle_deg)
{
    ...
    switch (channel) {
    case SERVO_CHANNEL_X:
        max_angle = SERVO_X_MAX_ANGLE_DEG;
        break;
    case SERVO_CHANNEL_Y:
        max_angle = SERVO_Y_MAX_ANGLE_DEG;
        break;
    default:
        return;
    }
    ...
    pulse = (float)SERVO_PULSE_MIN_US + (angle_deg / max_angle) *
