...
if __name__=="__main__":
    ...
    while True:
        with ScopedTiming("total",1):
            ...
            if len(hand_det_res) > 0:
                # 提取第一只手
                target_hand = hand_det_res[0]
                x1, y1, x2, y2 = target_hand[2], target_hand[3], target_hand[4], target_hand[5]

                # 计算中心并求偏移量
                hand_center_x = (x1 + x2) / 2.0
                hand_center_y = (y1 + y2) / 2.0
                offset_x = int(hand_center_x - screen_center_x)
                offset_y = int(hand_center_y - screen_center_y)

                # 加入死区：手在屏幕中心 40 像素内，认为不用动，避免云台疯狂抽搐
                if abs(offset_x) < 40: offset_x = 0
                if abs(offset_y) < 40: offset_y = 0
